...
public class BinaryTreeQ implements Iterable<Integer>{
    ...
    public Iterator<Integer> iterator() {
        return new TreeIteratorQ(this.root);
    }
    ...
    private Node addTree(Node current, Integer key){
        ...
        //If the key is smaller then then current nodes, go to the left node
            ...
            current.left = addTree(current.left, key);
        }
        //If the key is bigger then then current nodes, go to the right node
        else if(key > current.key){
            current.right= addTree(current.right, key);
        }
